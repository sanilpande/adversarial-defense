...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim=10, image_channels=3, hidden_dim=128):
        super(Generator, self).__init__()
        self.z_dim = z_dim
        self.gen_1 = self.make_gen(z_dim, hidden_dim * 4, kernel_size=4, stride=1)
        # Size 256x4x4
        self.gen_2 = self.make_gen(hidden_dim * 4, hidden_dim * 2, kernel_size=4, stride=2, padding=1)
        # Size 128x8x8
        self.gen_3 = self.make_gen(hidden_dim * 2, hidden_dim, kernel_size=4, stride=2, padding=1)
        # Size 64x16x16
        self.gen_4 = self.make_gen(hidden_dim, image_channels, kernel_size=4, stride=2, padding=1, final_layer=True)
        # Size 3x32x32

    def make_gen(self, input_channels, output_channels, kernel_size=4, stride=1, padding=0, final_layer=False):
        """Make a generator black with transpose convolutions."""
        if not final_layer:
...
class Discriminator(nn.Module):
    def __init__(self, image_channels=3, hidden_dim=64):
        ...
        self.disc_1 = self.make_disc(image_channels, hidden_dim)
        # Size 16x16x16
        self.disc_2 = self.make_disc(hidden_dim, hidden_dim * 2)
        # Size 32x8x8
        self.disc_3 = self.make_disc(hidden_dim * 2, hidden_dim * 4)
        # Size 64x4x4
        self.disc_4 = self.make_disc(hidden_dim * 4, 1, final_layer=True)
        # Size 1x1x1

    def make_disc(self, input_channels, output_channels, kernel_size=4, stride=2, final_layer=False):
